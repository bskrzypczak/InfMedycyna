...
def generuj_sinogram():
    ...
    global sinogram_frames, shared_slider
    sinogram_frames = []
    if shared_slider:
    ...
    if obraz is not None:
        try:
            ...
            shared_slider.setVisible(False)
            shared_slider_label.setText("Krok: 0")
        except Exception as e:
...
def animuj_rekonstrukcje():
    ...
    global reconstruction_frames, shared_slider, shared_slider_label
    reconstruction_frames = []
    try:
        krok_alpha_val = krok_alpha.value() if isinstance(krok_alpha, QSpinBox) else krok_alpha
        liczba_detektorow_val = liczba_detektorow.value() if isinstance(liczba_detektorow, QSpinBox) else liczba_detektorow
        ...
        for frame in klatki:
            ...
            reconstructed_image_label.setPixmap(pixmap)
            reconstruction_frames.append(pixmap)
            QApplication.processEvents()
            time.sleep(0.03)
        ...
        if len(reconstruction_frames) > 0 and len(sinogram_frames) == len(reconstruction_frames):
            ...
            def slider_moved(val):
                shared_slider_label.setText(f"Krok: {val}")
                reconstructed_image_label.setPixmap(reconstruction_frames[val])
                if sinogram_label and val < len(sinogram_frames):
                    sinogram_label.setPixmap(sinogram_frames[val])

            shared_slider.valueChanged.connect(slider_moved)
            shared_slider.setVisible(True)
            shared_slider_label.setVisible(True)
...
""")
    ...
    # Kontener na suwak pod ramkami
    global slider_layout, shared_slider, shared_slider_label
    slider_layout = QHBoxLayout()
    slider_layout.setContentsMargins(20, 10, 20, 10)

    shared_slider_label = QLabel("Krok: 0")
    shared_slider_label.setStyleSheet("color: white; font-size: 14px;")
    ...
    shared_slider.setStyleSheet("""
        ...
        QSlider::handle:horizontal {
            background: #007BFF; width: 12px; border-radius: 6px; margin: -5px 0;
        }
    """)

    slider_layout.addWidget(shared_slider_label)
    slider_layout.addWidget(shared_slider)
